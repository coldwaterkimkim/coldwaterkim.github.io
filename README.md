...
- 공개 사이트: `index.html`, `posts/index.html`, `posts/view.html`, `programs/index.html`, `programs/view.html`, `nasajab/index.html`, `guestbook.html`, `about.html`
...
- `guestbook`: 방명록
- `site_settings`: 홈 문구, 프로필 사진 URL, 홈 BGM URL/제목 같은 간단 설정
...
4. 미니 플레이어 아래 `MP3 바꾸기` 버튼으로 새 MP3를 업로드하면 `site_settings.bgm_audio_url`, `site_settings.bgm_audio_title`이 갱신된다.
5. 방문자는 저장된 프로필 사진과 BGM 제목 marquee를 보며, 브라우저가 허용하면 BGM은 접속 시 자동 재생된다. 자동재생이 막히면 첫 클릭/키 입력 뒤 다시 재생을 시도한다.
...
4. 사진/캡처/GIF를 `이미지`에 첨부한다.
5. 메모, 출처 링크, 잡힌 시각은 선택 사항이다. 제목과 내용은 나누지 않고 메모 하나만 남긴다.
6. `공개` 체크가 켜진 레코드만 방문자에게 보인다.

나사잡 첫 화면은 최신 항목 1개를 크게 보여주는 `TODAY'S 나사잡` table과, 그 아래 5개 단위 페이지네이션이 붙은 `나사잡 아카이브` table로 나뉜다. 아카이브 메모나 썸네일을 누르면 같은 페이지의 상단 큰 영역에서 확대되어 보인다.
