...
- 공개 홈은 PocketBase에서 `글방`, `나으 하루`, `프로그램실`, `나사잡`의 최신 공개 항목을 각각 3개씩 가져와 별도 table로 보여준다.
- 글 목록은 홈과 같은 shell 안에서 PocketBase `posts` 컬렉션의 `published` 글만 보여주며, 홈의 글방 최근 3개 table과 같은 최신순 기준을 쓴다.
- 글 상세 URL은 홈과 같은 shell 안에서 `slug`로 PocketBase 글을 조회하되, 단일 글만 보여주지 않고 전체 글을 최신순 timeline으로 렌더링한다. 특정 글 링크로 들어오면 해당 글이 `NOW READING` 상태로 첫 포커스를 받고, 그 위에는 더 최근 글, 아래에는 더 오래된 글이 이어진다. 포커스된 글은 빨간 테두리로만 강조하고 별도 노란 배경은 쓰지 않는다.
...
- 나으 하루 목록은 홈과 같은 shell 안에서 상단 월간 캘린더와 하단 하루 table로 보인다. 글이 있는 날짜는 캘린더에서 링크로 표시되고, 같은 날짜 기록이 여러 개면 캘린더에 개수가 표시된다. table row나 캘린더 날짜를 누르면 해당 기록의 slug로 상세 timeline에 들어간다.
- 나으 하루 상세 URL은 글방 상세와 같은 timeline 방식이다. 특정 하루 글이 `NOW READING` 상태로 포커스되고, 위아래로 다른 하루 글이 이어진다. 포커스된 글은 글방처럼 빨간 테두리만 쓴다.
...
- About / Contact는 일반 자기소개 페이지가 아니라 나무위키식 `김찬수` 문서로 보인다. 오른쪽 프로필 표, 목차, 번호 섹션, 섹션별 `[편집]` 링크를 쓰며, 로그인한 관리자는 공개 페이지의 `OWNER MODE`에서 섹션 추가/수정/삭제/위아래 이동과 프로필 표 row 편집을 할 수 있다. 섹션 본문은 글방/나으 하루와 같은 WYSIWYG Markdown 편집기로 작성하고, 저장값은 새 컬렉션 없이 `site_settings.about_wiki_document` JSON에 둔다.
...
- 나으 하루 작성기의 WYSIWYG Markdown 본문 이미지 업로드 경험도 글 편집기와 같다. 같은 날짜 `daily_entries`가 이미 있어도 새 작성은 기존 글에 합치지 않고 별도 레코드로 저장한다.
