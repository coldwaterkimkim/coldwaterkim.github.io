...
- 공용 BlockNote 편집기의 본문 이미지를 선택하면 `선택 이미지 자르기`를 열 수 있다. 자유/1:1/4:3/16:9 비율로 보이는 사각형만 지정하며 `media.file` 원본이나 PocketBase 레코드는 수정하지 않는다. 각 이미지 블록의 정규화 좌표가 본문 HTML `data-cwk-image-crop`에 저장되므로 같은 원본을 여러 번 넣어도 서로 다른 crop을 유지하고, 재편집하거나 `원본 전체로` 초기화할 수 있다. 글방·나으 하루·통합 글쓰기·프로그램 상세 본문이 이 공용 동작을 사용하며 About 나무마크 textarea와 앨범 정방형 타일 crop은 별도 흐름으로 유지한다.
