...
- `media`: 본문 이미지, 영상, 오디오, PDF 업로드. 글방/나으 하루/프로그램실/About 작성기의 BlockNote 파일 업로드가 이 컬렉션을 쓴다.
...
5. 본문은 같은 WYSIWYG Markdown 에디터에서 작성한다. 본문 이미지는 이미지 버튼, 드래그 앤 드롭, 붙여넣기로 넣고, `/video` 같은 파일 블록은 `media` 업로드를 거쳐 본문에 들어간다. 유튜브 링크는 붙여넣거나 비디오 블록의 URL 입력으로 넣으면 공개 화면에서 임베드 플레이어로 보인다.
