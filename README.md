...
방문자 카운터는 새로고침마다 숫자를 올리는 페이지뷰가 아니라, 같은 브라우저의 30분 내 활동을 1회 방문으로 묶는다. `TODAY`는 KST 날짜 기준이고, `TOTAL`은 전체 방문 세션 누적값에 2000을 더해 보여준다. 로그인한 관리자는 공개 화면에서 `TODAY` 옆의 위/아래 버튼으로 오늘 표시 최소값을 조절할 수 있고, 이 보정값은 `site_settings`의 `visitor_today_min_YYYY-MM-DD` 키에 저장된다.
