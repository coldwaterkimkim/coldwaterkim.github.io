import{K as e,N as t,P as n,Y as r,b as i,g as a,nt as o,r as s,st as c,v as l}from"../pb-UUPSdoxh.js";import"../site-DeC2s_9n.js";import{r as u,t as d}from"../media-embeds-h4FUltIX.js";var f=new URLSearchParams(window.location.search).get(`slug`),p=e();f?m(f):b(`글을 찾을 수 없습니다.`,`Post not found`);async function m(e){try{y(e);let i=await t(e,p);if(i.status!==`published`&&!p){b(`이 글은 아직 발행되지 않았습니다.`,`Not published`);return}document.title=`${i.title} — coldwaterkim`,document.getElementById(`timeline-title`).textContent=`글방`,document.getElementById(`timeline-note`).innerHTML=`<b>${l(i.title)}</b> · 단일 글 페이지`;let o=p?await n([i.id]):{};p&&(document.getElementById(`owner-tools`).innerHTML=`
                        <div class="owner-bar">
                            <b>OWNER MODE</b> ·
                    ...
                    `,document.getElementById(`deletePostLink`).addEventListener(`click`,async e=>{if(e.preventDefault(),confirm(`이 글을 삭제할까요? 이 작업은 되돌릴 수 없습니다.`))try{await a(i.id),window.location.href=`index.html`}catch(e){alert(`삭제 실패: `+s(e))}}),document.getElementById(`logoutLink`).addEventListener(`click`,e=>{e.preventDefault(),r(),window.location.reload()})),g(i,o[i.id]),p||h(i)}catch(e){b(s(e),`Post not found`)}}function h(e){let t=()=>{c(e).catch(e=>{console.warn(`Post view count failed:`,s(e))})};if(!(document.documentElement.classList.contains(`entry-gate-pending`)||document.documentElement.classList.contains(`entry-gate-open`))||window.__coldwaterkimEntryAdmitted===!0||document.documentElement.dataset.entryAdmitted===`true`){t();return}window.addEventListener(`coldwaterkim:entry-admitted`,t,{once:!0})}function g(e,t=null){let n=document.getElementById(`post-timeline`);n.innerHTML=_(e,t),d(n)}function _(e,t=null){let n=o(e),r=e.status===`published`?``:` <small class="note">[초안]</small>`,a=p?` · 조회수 <span class="post-view-count">${v(t)}</span>`:``;return`
