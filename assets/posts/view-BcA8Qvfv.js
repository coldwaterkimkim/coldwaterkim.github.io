import{C as e,H as t,L as n,M as r,W as i,Y as a,at as o,g as s,i as c,v as l,y as u}from"../pb-EGplk_ig.js";import"../site-7yX1ceV2.js";var d=new URLSearchParams(window.location.search).get(`slug`),f=t();d?p(d):x(`글을 찾을 수 없습니다.`,`Post not found`);async function p(t){try{b(t);let a=await r(t,f);if(a.status!==`published`&&!f){x(`이 글은 아직 발행되지 않았습니다.`,`Not published`);return}document.title=`${a.title} — coldwaterkim`,document.getElementById(`timeline-title`).textContent=`글방`,document.getElementById(`timeline-note`).innerHTML=`<b>${l(a.title)}</b>`;let o=m(f?await e():await n(),a);f&&(document.getElementById(`owner-tools`).innerHTML=`
                        <div class="owner-bar">
                            <b>OWNER MODE</b> ·
                    ...
                    `,document.getElementById(`deletePostLink`).addEventListener(`click`,async e=>{if(e.preventDefault(),confirm(`이 글을 삭제할까요? 이 작업은 되돌릴 수 없습니다.`))try{await s(a.id),window.location.href=`index.html`}catch(e){alert(`삭제 실패: `+c(e))}}),document.getElementById(`logoutLink`).addEventListener(`click`,e=>{e.preventDefault(),i(),window.location.reload()})),h(o,a.id),_(a.id),v(a.id)}catch(e){x(c(e),`Post not found`)}}function m(e,t){return o(e.some(e=>e.id===t.id)?e:[...e,t])}function h(e,t){let n=document.getElementById(`post-timeline`);if(e.length===0){n.innerHTML=`<p>아직 글이 없습니다.</p>`;return}n.innerHTML=e.map(e=>g(e,e.id===t)).join(``)}function g(e,t){let n=a(e),r=e.status===`published`?``:` <small class="note">[초안]</small>`,i=t?`<div class="timeline-focus-badge">★ NOW READING ★</div>`:``,o=f?`
