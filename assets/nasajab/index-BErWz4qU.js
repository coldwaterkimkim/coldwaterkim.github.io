        ...
        `,S.innerHTML=``;return}let n=Math.max(1,Math.ceil(e.length/d));h=Math.min(Math.max(h,1),n);let r=(h-1)*d;x.innerHTML=e.slice(r,r+d).map((t,n)=>{let a=e.length-r-n,o=I(t),s=F(t),c=g===t.id,d=p?`<td class="nasajab-owner-cell">${P(t)}</td>`:``;return`
            <tr class="${c?`nasajab-archive-row--selected`:``}">
                ...
                <td>
                    ...
                    <small class="nasajab-mobile-date">${u(l(i(t)))}</small>
                </td>
                <td align="center" class="nasajab-archive-date">${u(l(i(t)))}</td>
                ${d}
            </tr>
