...
필수 BGM 입장 화면 구현은 보존되어 있지만 현재 공개 사이트에서는 잠시 비활성화했습니다. 방문자는 입장 화면 없이 기존 홈페이지로 바로 들어오며, 나중에 방향을 확정하면 공개 HTML의 `entry-gate-disabled` 표시를 `entry-gate-pending`으로 되돌려 다시 활성화할 수 있습니다.

사이트 이동 시 왼쪽 프로필과 BGM 플레이어는 유지하고, 오른쪽 본문 영역만 교체되도록 만들었습니다. 덕분에 글방, 나으 하루, 파일 작업실인 프로그램실, 나사잡, 방명록을 오가도 음악이 끊기지 않습니다.

사이드바의 `My WebRing`은 공개된 글, 하루, 앨범 미디어, 나사잡을 이어 보는 탐험 기능입니다. `Prev`와 `Next`는 기록 사이를 순환하고, `Random`은 사진 수가 다른 기록을 압도하지 않도록 콘텐츠 종류를 먼저 고른 뒤 한 항목을 보여줍니다.
...
- 방문자 수, 방명록, 글 조회수 관리
- 공개 기록 WebRing 탐험과 개인정보를 저장하지 않는 익명 이용 지표
- 글·하루 본문을 첫 HTML부터 제공하는 검색/AI 검색용 상세 URL, 사이트맵, 구조화 데이터
