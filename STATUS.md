...
- 2026-08-03 편집기 발행 후 이동, 전역 붙여넣기 중복 방지, Uppy/tusd 재개 업로드를 소스와 아이맥 배포 도구에 통합했다. 격리한 운영 DB 사본에서 migration 적용·rollback·재적용과 DB 무결성을 확인했고, tus 업로드를 70,000바이트에서 끊은 뒤 HEAD 오프셋부터 이어 받아 원본 체크섬이 같은 `media.file`을 만들었다. OWNER 사용자 인증, 같은 upload id의 중복 finalize, Uppy의 64MB 초과 클라이언트 업로드도 통과했다. 격리 브라우저에서는 두 이미지를 동시에 두 번 붙여넣어도 2개 블록만 생겼고, 글방·나으 하루·프로그램실 발행이 각각 일반 상세 화면으로 이동했으며 콘솔 오류는 없었다. 이 검증은 운영 데이터와 운영 서비스를 건드리지 않았고, 실제 아이맥 런타임 배포는 아직 하지 않았다.
