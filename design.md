...
`Ask Me`는 방명록의 인사 공간과 분리된 질문함이다. 입력부는 시각적 label 없이 질문 `textarea`, `이 질문은 공개하지 않기` checkbox, 기본 submit button만 둔다. 닉네임·제목·카테고리·이메일·첨부파일·댓글·좋아요는 받지 않는다. 홈에는 최근 글 5개와 앨범 사이에 같은 최소 질문창만 두며 placeholder는 `평소 궁금했지만 물어보지 못한 것`이다.
