...
`Ask Me`는 방명록의 인사 공간과 분리된 질문함이다. 입력부는 시각적 label 없이 질문 `textarea`, `이 질문은 공개하지 않기` checkbox, 기본 submit button만 둔다. 닉네임·제목·카테고리·이메일·첨부파일·댓글·좋아요는 받지 않는다. 홈에는 최근 글 5개와 앨범 사이에 같은 최소 질문창만 두되, 주변 섹션처럼 회색 헤더와 각진 폼을 가진 표 문법을 유지한다. 헤더에는 `Ask Me`와 `질문 목록` 링크를 두고 placeholder는 `평소 궁금했지만 물어보지 못한 것`이다.
