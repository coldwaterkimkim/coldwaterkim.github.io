...
- `daily_entries`: 나으 하루. 글방과 별도 저장소를 쓰되 Quill 작성 경험은 같고, 구분 단위는 글이 아니라 `day_key` 기준 하루다.
...
4. 글방과 같은 Quill 작성 화면에서 제목, 기록 날짜, 본문, 본문 이미지를 입력한다. 기록 날짜는 기본값이 오늘이지만 과거 날짜로 바꿀 수 있다.
...
- `admin/daily.html`: 글방 작성기와 같은 Quill 경험으로 `daily_entries`를 쓰고 같은 날짜 저장은 append 처리하는 owner 화면
