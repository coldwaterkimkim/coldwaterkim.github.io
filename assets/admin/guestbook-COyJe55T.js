import{D as e,U as t,W as n,b as r,p as i,q as a,r as o,v as s}from"../pb-Be3WJXcJ.js";/* empty css               */n()||(window.location.replace(`/admin/login.html`),await new Promise(()=>{})),document.getElementById(`logoutBtn`).addEventListener(`click`,e=>{e.preventDefault(),a(),window.location.href=`/admin/login.html`});var c=1;window.confirmDelete=p,window.goToPage=f;function l(e,t){let n=document.getElementById(`alert`);n.textContent=e,n.className=`alert alert-${t}`,n.style.display=`block`,setTimeout(()=>n.style.display=`none`,5e3)}async function u(n=1){let i=document.querySelector(`#guestbookTable tbody`);i.innerHTML=`<tr><td colspan="4" class="loading">불러오는 중</td></tr>`;try{let a=await e(n,20);if(c=n,document.getElementById(`stats`).textContent=`전체 방명록 수: ${a.totalItems}개`,a.items.length===0){i.innerHTML=`<tr><td colspan="4">방명록이 비어있습니다.</td></tr>`;return}i.innerHTML=a.items.map(e=>`
          <tr>
            <td><b>${s(e.name)}</b></td>
            <td>${s(e.message)}</td>
            <td>${r(t(e))}</td>
