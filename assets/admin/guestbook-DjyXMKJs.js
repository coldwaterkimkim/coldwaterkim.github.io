import{$ as e,A as t,Q as n,i as r,m as i,nt as a,x as o,y as s}from"../pb-CaVVfgGS.js";/* empty css               */e()||(window.location.replace(`/admin/login.html`),await new Promise(()=>{})),document.getElementById(`logoutBtn`).addEventListener(`click`,e=>{e.preventDefault(),a(),window.location.href=`/admin/login.html`});var c=1;window.confirmDelete=p,window.goToPage=f;function l(e,t){let n=document.getElementById(`alert`);n.textContent=e,n.className=`alert alert-${t}`,n.style.display=`block`,setTimeout(()=>n.style.display=`none`,5e3)}async function u(e=1){let i=document.querySelector(`#guestbookTable tbody`);i.innerHTML=`<tr><td colspan="4" class="loading">불러오는 중</td></tr>`;try{let r=await t(e,20);if(c=e,document.getElementById(`stats`).textContent=`전체 방명록 수: ${r.totalItems}개`,r.items.length===0){i.innerHTML=`<tr><td colspan="4">방명록이 비어있습니다.</td></tr>`;return}i.innerHTML=r.items.map(e=>`
          <tr>
            <td><b>${s(e.name)}</b></td>
            <td>${s(e.message)}</td>
            <td>${o(n(e))}</td>
