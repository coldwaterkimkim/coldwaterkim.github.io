...
`codex/records-v2-preview` 워크트리의 `/records/`는 사용자 검토용 새 방향이다. 운영 디자인 교체는 아직 승인되지 않았다. 노란 헤더, 크림색 바탕, 파란 링크, 각진 버튼을 유지하면서 모바일 단일 피드를 사용한다. PC는 기존 1140px 홈페이지의 프로필 사이드바·마키·블링크 배너·소개 문구·상단 탐색을 유지하고 본문 미리보기 영역만 피드로 교체한다. 모바일 프로필은 운영 홈페이지의 사진·TODAY IS·전체 프로필 표 배치를 공통 적용하고 접지 않는다. 소개를 유지하며 좌측 상단 햄버거로 전체 페이지를 탐색한다. 하단에는 홈·앨범·글방만 남긴다. 기존 원문은 별도 펼치기 없이 일부를 바로 보여주고 긴 내용은 더 보기 하나로 확장한다. 홈은 글방·나으 하루의 개별 기록을 최초 발행 최신순으로 섞는다. 사진은 가로 스와이프하며, 사진별 코멘트가 없으면 공통 본문을 표시한다. 작성은 일반 텍스트와 첨부 중심으로 바꾼다. 복잡한 기존 문서는 원문 보존 영역을 유지한다. 아래 운영 기준은 승인 전까지 기존 화면에 계속 적용한다.
