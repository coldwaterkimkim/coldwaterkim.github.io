...
상태: **DISABLED**. 2026-09-16 사용자 요청으로 해제했다. 사이트가 지원서의 참고 링크로 사용 중이므로 기존 공개 사이트를 유지한다. 새 명시적 요청 없이 다시 활성화하지 않는다.

활성 설정이 점검 후보와 일치함을 확인한 후 저장된 정상 설정을 검증·복원·reload했다. 메인/www/앨범/About/API health HTTP 200과 브라우저 정상 홈을 확인했다. 철학서·동결 문서·기념사진은 변경하지 않았다.
...
저장소 `deploy/imac/Caddyfile`은 정상 서비스 기준으로 유지했다. 점검 후보는 비활성 사본이며 현재 활성 Caddyfile은 정상 설정으로 복원되어 있다.
...
이 문서는 수동 점검 활성화 및 해제 이력이다. 새 Sketchbook + Crayon 홈페이지는 아직 구현하지 않았다. 아래 보관된 과거 503 검증을 현재 상태로 해석하지 않는다. 현재 공개 응답은 정상 서비스 200이다.
