...
- 목록은 10개 단위이며 `[ 1 2 3 다음 ▶ ]` 형태의 옛 게시판 pagination을 쓴다. OWNER는 같은 공개 페이지에서 실제 질문을 읽고 답변을 저장·수정·삭제한다.
