...
`Ask Me`는 방명록의 인사 공간과 분리된 질문함이다. 입력부는 시각적 label 없이 질문 `textarea`, `이 질문은 공개하지 않기` checkbox, 기본 submit button만 둔다. 닉네임·제목·카테고리·이메일·첨부파일·댓글·좋아요는 받지 않는다. 홈에는 최근 글 8개와 앨범 사이에 같은 최소 질문창만 두며 질문 미리보기나 목록은 노출하지 않는다. 별도 카드로 분리하지 않고 주변 섹션처럼 회색 헤더, table row, 각진 폼 문법을 유지한다. 헤더에는 `Ask Me`와 전체 질문을 확인하는 `질문 목록` 링크를 두고 placeholder는 `질문이 있다면`이다. 질문과 답변의 공개 목록, 미답변·비공개 상태 문구, 작성자 열람 기능은 Ask Me 탭에서 유지한다. Ask Me 본문 제목은 `???`를 쓰되 상단 탐색 메뉴의 `Ask Me` 명칭은 유지한다.
