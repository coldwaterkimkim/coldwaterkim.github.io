...
- 나으 하루는 글방과 별도인 PocketBase `daily_entries` 컬렉션을 쓴다. 작성 화면은 글방과 같은 WYSIWYG Markdown 작성 경험을 유지하지만 저장 단위는 글이 아니라 `day_key` 기준 하루다. 새 작성의 기록 날짜는 오늘로 기본 설정되지만 직접 바꿀 수 있고, 같은 날짜에 새로 저장하면 새 레코드를 만들지 않고 기존 하루 본문 아래에 이어 붙인다.
