...
`Ask Me`는 방명록의 인사 공간과 분리된 질문함이다. 입력부는 시각적 label 없이 질문 `textarea`, `이 질문은 공개하지 않기` checkbox, 기본 submit button만 둔다. 닉네임·제목·카테고리·이메일·첨부파일·댓글·좋아요는 받지 않는다.

- 질문은 최초 작성 순서를 기준으로 최신 질문부터 보여주며, 답변 시각 때문에 위치를 바꾸지 않는다.
- 작성자 이름은 서버의 한국 시간과 전체 질문 순번으로 자동 생성한다. 05–06시는 `너무 일찍 깬`, 07–09시는 `정신 차리는`, 10–12시는 `배고픈`, 13–16시는 `딴짓 중인`, 17–19시는 `할 말 생긴`, 20–22시는 `오늘이 아쉬운`, 23–01시는 `잠 안 자는`, 02–04시는 `진짜 안 자는 질문자 n`이다.
- 공개 미답변 질문과 비공개 질문도 목록에서 자리를 유지하되 실제 원문은 공개 API에 내려주지 않고 합의된 상태 문구만 표시한다. 공개 질문에 OWNER 답변이 저장된 경우에만 `Q.` 원문과 남색 double border-left의 `A.` 답변을 함께 보여준다.
