...
`codex/records-v2-preview` 워크트리의 `/records/`는 사용자 검토용 새 방향이다. 운영 디자인 교체는 아직 승인되지 않았다. 노란 헤더, 크림색 바탕, 파란 링크, 각진 버튼을 유지하면서 모바일 단일 피드와 하단 탐색을 사용한다. 홈은 글방·나으 하루의 개별 기록을 최초 발행 최신순으로 섞는다. 사진은 가로 스와이프하며, 사진별 코멘트가 없으면 공통 본문을 표시한다. 작성은 일반 텍스트와 첨부 중심으로 바꾼다. 복잡한 기존 문서는 원문 보존 영역을 유지한다. 아래 운영 기준은 승인 전까지 기존 화면에 계속 적용한다.
