...
const sources = {
  ...
  programs: [{ id: 'r1', title: '프로그램', slug: 'program', created: '2026-08-18T10:00:00Z', first_published_at: '2026-08-24T10:00:00Z' }],
...
assert.equal(entries.find(entry => entry.category === 'program').date, sources.programs[0].first_published_at);
...
for (const category of ['post', 'daily', 'program', 'nasajab', 'guestbook']) {
  const entry = entries.find(item => item.category === category);
  assert.notEqual(entry.date, sources[category === 'post' ? 'posts' : category === 'daily' ? 'daily' : category === 'program' ? 'programs' : category][0]?.updated, `${category} edits must not reorder an already-public entry`);
}
assert.deepEqual(entries.map(entry => entry.category), ['post', 'program', 'nasajab', 'daily', 'guestbook']);
...
for (const collectionName of ['posts', 'daily_entries', 'programs', 'nasajab', 'guestbook']) {
...
for (const loader of ['getPublishedProgramSummaries', 'getPublishedNasajabSummaries', 'getAnsweredGuestbookSummaries']) {
