import{G as e,O as t,S as n,d as r,f as i,j as a,n as o,r as s,s as c,u as l,y as u}from"../pb-BpG7gW_X.js";import"../site-Ru_1E4HZ.js";var d=5,f=t(),p=new URLSearchParams(window.location.search).has(`demo`),m=[],h=1,g=G(),_=``,v=document.getElementById(`nasajabFeatured`),y=document.getElementById(`nasajabArchive`),b=document.getElementById(`nasajabPagination`),x=document.getElementById(`nasajabOwnerPanel`),S=document.getElementById(`nasajabOwnerHead`),C=document.getElementById(`nasajabOwnerStatus`),w=document.getElementById(`nasajabForm`),T=document.getElementById(`nasajabFormTitle`),E=document.getElementById(`newNasajabButton`),D=document.getElementById(`cancelNasajabEdit`),O={image:document.getElementById(`nasajabImage`),memo:document.getElementById(`nasajabMemo`),sourceUrl:document.getElementById(`nasajabSourceUrl`),displayAt:document.getElementById(`nasajabDisplayAt`),isPublic:document.getElementById(`nasajabIsPublic`)};f&&(x.hidden=!1,S.hidden=!1,W(`OWNER MODE: 사진 하나만 올려도 위쪽에 바로 뜸.`)),E?.addEventListener(`click`,()=>{z({hidden:!1}),w.scrollIntoView({block:`start`}),O.image.focus()}),D?.addEventListener(`click`,()=>z({hidden:!0})),w?.addEventListener(`submit`,async e=>{e.preventDefault(),await I()}),y?.addEventListener(`click`,async e=>{let t=e.target.closest(`[data-nasajab-action]`);if(t){let e=t.getAttribute(`data-nasajab-id`),n=t.getAttribute(`data-nasajab-action`);n===`edit`&&await L(e),n===`delete`&&await R(e);return}let n=e.target.closest(`[data-nasajab-pick]`);n&&(e.preventDefault(),g=n.getAttribute(`data-nasajab-pick`)||``,g?window.location.hash=g:history.replaceState(null,``,window.location.pathname+window.location.search),A(),v.scrollIntoView({block:`start`}))}),v?.addEventListener(`click`,async e=>{let t=e.target.closest(`[data-nasajab-action]`);if(t){let e=t.getAttribute(`data-nasajab-id`),n=t.getAttribute(`data-nasajab-action`);n===`edit`&&await L(e),n===`delete`&&await R(e);return}}),b?.addEventListener(`click`,e=>{let t=e.target.closest(`[data-nasajab-page]`);if(!t)return;e.preventDefault();let n=Number.parseInt(t.getAttribute(`data-nasajab-page`)||`1`,10);Number.isFinite(n)&&(h=n,M())}),window.addEventListener(`hashchange`,()=>{g=G(),A()}),k();async function k(){if(v.innerHTML=`<p>나사잡 불러오는 중...</p>`,y.innerHTML=`<tr><td colspan="5">아카이브 불러오는 중...</td></tr>`,p){m=Z(),W(`DEMO MODE: 실제 CMS 저장 없이 화면만 보는 중.`),A();return}try{m=f?await i():await n(),A()}catch(e){F(e)}}function A(){if(!m.length){P();return}let e=m[0],t=g?m.find(e=>e.id===g):null;j(t||e,!!t),M()}function j(e,t=!1){let n=t?`ARCHIVE PICK`:`TODAY'S 나사잡`,r=H(e),i=V(e),o=K(a(e)),s=e.source_url?`<p class="nasajab-source">source: <a href="${X(e.source_url)}" target="_blank" rel="noopener">${l(J(e.source_url))}</a></p>`:``,c=f?B(e):``;v.innerHTML=`
        <table border="1" cellspacing="0" cellpadding="6" width="100%" class="nasajab-today-table nasajab-today-table--stacked">
            ...
            <tr>
                <td class="nasajab-featured-image-cell">
                    ${r?`<img src="${X(r)}" alt="${X(i)}">`:`<div class="nasajab-missing-image">이미지 없음</div>`}
                </td>
                <td class="nasajab-featured-text-cell">
                    <div class="nasajab-featured-memo">${Y(i)}</div>
                    ${s}
    ...
    `}function M(){let e=m,t=f?5:4;if(!e.length){y.innerHTML=`
            <tr>
                <td colspan="${t}">
                    아직 아래에 쌓인 것이 없습니다.
        ...
        `,b.innerHTML=``;return}let n=Math.max(1,Math.ceil(e.length/d));h=Math.min(Math.max(h,1),n);let i=(h-1)*d;y.innerHTML=e.slice(i,i+d).map((t,n)=>{let o=e.length-i-n,s=H(t),c=V(t),u=g===t.id,d=f?`<td class="nasajab-owner-cell">${B(t)}</td>`:``;return`
            <tr class="${u?`nasajab-archive-row--selected`:``}">
                ...
                <td align="center" class="nasajab-thumb-cell">
                    <a href="#${X(t.id)}" data-nasajab-pick="${X(t.id)}">
                        ${s?`<img src="${X(s)}" alt="${X(c)} thumbnail">`:`<span class="note">no img</span>`}
                    </a>
                </td>
                <td>
                    <a href="#${X(t.id)}" data-nasajab-pick="${X(t.id)}">${l(c)}</a>
                    ${t.is_public===!1?` <small class="note">[비공개]</small>`:``}
                    ${u?` <small class="note">[NOW]</small>`:``}
                    <small class="nasajab-mobile-date">${l(r(a(t)))}</small>
                </td>
                <td align="center" class="nasajab-archive-date">${l(r(a(t)))}</td>
                ${d}
            </tr>
        `}).join(``),N(n)}function N(e){if(e<=1){b.innerHTML=``;return}let t=[];for(let n=1;n<=e;n+=1)t.push(n===h?`<b>${n}</b>`:`<a href="#" data-nasajab-page="${n}">${n}</a>`);b.innerHTML=`[ ${t.join(` `)} ]`}function P(){let e=f?5:4;v.innerHTML=`
    ...
    `,y.innerHTML=`<tr><td colspan="${r}">${l(t)}</td></tr>`,b.innerHTML=``,W(`CMS 확인 필요: ${n}`,`error`)}async function I(){if(!f)return;let t=O.image.files.length>0;if(!_&&!t){W(`이미지 하나는 꼭 있어야 함. 사진 하나만 띵똥 올리면 됨.`,`error`);return}let n=new FormData(w);n.set(`is_public`,O.isPublic.checked?`true`:`false`),t||n.delete(`image`),U(n,`memo`),U(n,`source_url`),O.displayAt.value?n.set(`display_at`,new Date(O.displayAt.value).toISOString()):n.delete(`display_at`),W(_?`나사잡 수정 중...`:`나사잡 저장 중...`);try{let t=_?await e(_,n):await s(n);W(`${V(t)} 저장 완료.`,`success`),z({hidden:!0}),g=t.id,window.location.hash=t.id,await k()}catch(e){W(`저장 실패: ${o(e)}`,`error`)}}async function L(e){if(!f||!e)return;let t=m.find(t=>t.id===e);t&&(_=t.id,w.hidden=!1,T.textContent=`✎ 나사잡 수정: ${V(t)}`,O.image.value=``,O.memo.value=t.memo||``,O.sourceUrl.value=t.source_url||``,O.displayAt.value=q(a(t)),O.isPublic.checked=t.is_public!==!1,W(`수정 모드. 새 이미지를 고르면 기존 이미지가 바뀜.`),w.scrollIntoView({block:`start`}),O.memo.focus())}async function R(e){if(!f||!e)return;let t=V(m.find(t=>t.id===e));if(confirm(`${t}을 삭제할까?\n이 작업은 되돌릴 수 없습니다.`))try{await c(e),g===e&&(g=``,history.replaceState(null,``,window.location.pathname+window.location.search)),W(`${t} 삭제 완료.`,`success`),await k()}catch(e){W(`삭제 실패: ${o(e)}`,`error`)}}function z(e={}){w&&(_=``,w.reset(),T.textContent=`✚ 새 나사잡 올리기`,O.displayAt.value=q(new Date().toISOString()),O.isPublic.checked=!0,w.hidden=e.hidden??!0)}function B(e){return`
