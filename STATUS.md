...
- About / Contact는 일반 자기소개 페이지가 아니라 나무위키식 `김찬수` 문서로 보인다. 개요보다 먼저 보이는 프로필 표, 목차, 번호 섹션, 섹션별 `[편집]` 링크를 쓰며, 로그인한 관리자는 각 섹션 제목의 `↑`/`↓` 버튼으로 순서를 즉시 바꾸고 섹션 추가/수정/삭제와 프로필 표 row 편집을 할 수 있다. 섹션 본문은 글방/나으 하루와 같은 WYSIWYG Markdown 편집기로 작성하고, 저장값은 `site_settings.about_wiki_document` JSON에 둔다. `site_settings.value`는 긴 문서도 저장하도록 500만 자까지 허용하고, 편집 중에는 버전 배포 자동 새로고침을 미뤄 작성 내용을 보호한다.
