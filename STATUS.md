...
- 공개 글은 `/posts/{slug}/`, 하루는 `/daily/{YYYY-MM-DD}/`를 검색 기준 URL로 쓴다. iMac PocketBase가 발행된 DB 본문을 요청 시 정적 viewer shell에 주입해 JavaScript 실행 전부터 제목·날짜·본문·canonical·공유 메타·`BlogPosting` 구조화 데이터를 제공한다. 발행/수정/초안 전환은 다음 요청부터 즉시 반영된다. 예전 `view.html?...` 주소는 OWNER 초안 미리보기와 기존 링크 호환을 위해 유지하되 `noindex,follow`로 검색 중복을 막는다.
...
- `robots.txt`는 특정 검색 서비스만 선별하지 않고 모든 봇에게 공개 영역을 허용한다. 동적 `sitemap.xml`은 주요 정적 페이지와 발행된 글·하루만 포함하며 관리자·API·방명록·초안·예전 viewer 주소는 검색 대상에서 제외한다.
...
- 로그인한 관리자는 공개 사이트를 그대로 보면서 홈 문구 편집, 프로필 사진 클릭 업로드, 홈 BGM MP3 추가, Home의 통합 글쓰기 허브, 글방의 새 글/수정/삭제, 나으 하루의 새 하루/수정/삭제, 글 상세의 수정/삭제, 프로그램실의 새 프로그램/수정/삭제, 나사잡의 이미지 업로드/수정/삭제, 방명록 답글 작성·수정·삭제와 방명록 삭제 같은 `OWNER MODE` 권한을 추가로 본다. 통합 글쓰기 허브는 글방/나으 하루/프로그램실 새 레코드 생성을 빠르게 시작하는 추가 진입점이며 기존 각 페이지 작성/수정 플로우를 대체하지 않는다. 프로그램실 새 레코드는 공개 자료실 항목으로 저장된다. 임시 저장은 편집기에 머물고, 발행 저장이 성공하면 글방은 글 상세, 나으 하루는 해당 날짜 상세, 프로그램실은 프로그램 상세의 일반 보기 화면으로 즉시 이동한다.
- 프로필 사진과 홈 BGM은 별도 컬렉션을 만들지 않고 기존 `media` 컬렉션과 `site_settings`를 쓴다. 프로필 사진은 `site_settings.profile_photo_url`에 저장한다. BGM은 `site_settings.bgm_playlist`를 최신 업로드순 플레이리스트로 쓰고, 기존 `site_settings.bgm_audio_url`, `site_settings.bgm_audio_title`은 최신곡 호환용으로 같이 갱신한다. `site_settings.bgm_schedule`은 한국 시간 기준 새벽/아침/낮/저녁/밤의 경계와 곡별 중복 배정을 저장하며, 편성 정보가 없거나 새로 업로드한 곡은 기존 동작을 보존하도록 모든 시간대에 자동 배정한다. 첫 진입과 곡 종료 때 현재 시간대 후보에서 랜덤으로 고르고, 후보가 비어 있으면 전체 목록으로 안전하게 돌아가며, 곡이 2개 이상이면 같은 곡을 바로 반복하지 않는다. 시간대가 바뀌어도 재생 중인 곡은 끊지 않고 다음 선곡부터 새 편성을 적용한다. 로그인한 관리자는 BGM 아래 `BGM 편성표` 버튼으로 본문에 펼쳐지는 곡×시간대 체크표, 시간대 이름/시작 시각, 미리듣기와 MP3 추가를 관리한다. 자동재생이 막히면 `BGM ON` 버튼과 첫 사용자 입력 뒤 다시 시도한다.
...
- 공용 BlockNote 편집기의 본문 이미지를 한 번 선택하면 이미지 바로 위의 도구막대에서 가위 모양 `이미지 자르기`를 열 수 있다. 편집기 맨 위에 고정된 crop 버튼은 두지 않아 긴 본문의 아래쪽 사진도 스크롤을 되돌리지 않고 작업한다. 자유/1:1/4:3/16:9 비율로 보이는 사각형만 지정하며 `media.file` 원본이나 PocketBase 레코드는 수정하지 않는다. 각 이미지 블록의 정규화 좌표가 본문 HTML `data-cwk-image-crop`에 저장되므로 같은 원본을 여러 번 넣어도 서로 다른 crop을 유지하고, 재편집하거나 `원본 전체로` 초기화할 수 있다. 글방·나으 하루·통합 글쓰기·프로그램 상세 본문이 이 공용 동작을 사용하며 About 나무마크 textarea와 앨범 정방형 타일 crop은 별도 흐름으로 유지한다.
- 글방/나으 하루/프로그램실/About의 WYSIWYG 본문은 작성기와 공개 화면에서 같은 rich-content 보정 규칙을 쓴다. 빈 문단은 발행 화면에서도 의도한 빈 줄로 남기고, 이미지 붙여넣기/드래그/파일 선택은 공용 업로드 조정기를 거치며 여러 미디어는 Finder/사진 앱이 전달한 선택 순서를 그대로 보존해 본문에 들어간다.
- 공개 글/나으 하루/프로그램 상세 본문은 저장된 HTML을 렌더링한 뒤 `js/media-embeds.js`와 `css/styles.css`에서 Markdown 변환 요소와 미디어 요소를 보정한다. table은 border와 셀 padding을 가진 실제 표로 보이고, video/audio는 컨트롤을 가진 플레이어로, 유튜브 URL 기반 video 블록은 iframe 임베드로 보인다. PocketBase 영상 원본은 그대로 보존하고, 아이맥 백그라운드 작업이 포스터 JPEG와 1280px 이하 H.264/AAC Fast Start MP4를 별도 생성한다. 공개 화면은 준비된 파생본만 재생하며 아직 처리되지 않았거나 실패한 영상은 원본으로 fallback한다. code block, blockquote, list, hr도 같은 본문 렌더링 규칙을 따른다.
