...
단순 페이지는 marquee 없이 760px table만 써도 된다.

### Sidebar

홈의 사이드바는 공개 사이트의 네비게이션 원형이다.
...
아이콘 버튼, pill nav, sticky modern sidebar로 바꾸지 않는다.
...
글 상세는 모던 블로그보다 조금 투박한 문서여야 한다.

- 본문 가독성을 위해 `line-height: 1.8`, `font-size: 15px`는 허용한다.
- 코드/blockquote는 단순 회색 박스와 왼쪽 선 정도만 쓴다.
- 하단에는 텍스트 링크로 `글 목록으로`, `Home`을 둔다.
...
3. 복잡한 페이지면 홈의 2-column shell을 가져온다.
4. 단순 페이지면 760px one-cell table로 만든다.
