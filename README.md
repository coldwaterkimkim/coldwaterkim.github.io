...
- `posts`: 글. 이미지는 Quill 본문 안에 직접 넣고, 별도 대표 이미지 로직은 쓰지 않는다.
...
5. 방문자는 저장된 프로필 사진과 BGM 제목 marquee를 보며, 브라우저가 허용하면 BGM은 접속 시 자동 재생된다. 자동재생이 막히면 `BGM ON` 버튼과 첫 사용자 입력 뒤 다시 재생을 시도한다.
...
6. 상세페이지용 긴 이야기, 사용법, 제작 배경, 스크린샷은 Quill 자유 본문에 넣는다.
