import{E as e,P as t,U as n,V as r,c as i,f as a,l as o,r as s,v as c,x as l,y as u}from"../pb-D2QCuDy_.js";import"../site-DnU6m-Az.js";var d=new URLSearchParams(window.location.search).get(`slug`),f=r();d?p(d):x(`나으 하루를 찾을 수 없습니다.`,`Daily not found`);async function p(r){try{b(r);let i=await e(r,f);if(i.status!==`published`&&!f){x(`이 하루는 아직 발행되지 않았습니다.`,`Not published`);return}document.title=`${i.title} — coldwaterkim`,document.getElementById(`timeline-title`).textContent=`나으 하루`,document.getElementById(`timeline-note`).innerHTML=`<b>${c(i.title)}</b>`;let o=m(f?await l():await t(),i);f&&(document.getElementById(`owner-tools`).innerHTML=`
                        <div class="owner-bar">
                            <b>OWNER MODE</b> ·
