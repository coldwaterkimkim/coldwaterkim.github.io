...
PocketBase `v0.23.5` 기본 HTTP 읽기/쓰기 제한시간 3분은 1GB 안팎의 영상이 2GB 파일 한도 이내여도 느린 업로드에서 실패하게 한다. 아이맥 운영 바이너리는 같은 `v0.23.5`를 기반으로 `--httpRequestTimeout=30m`과 tusd `v2.10.0` 재개 업로드 라우트를 함께 넣는다. 64MB 이상 영상은 Uppy가 파일을 3개 부분으로 병렬 전송하고 tus 결합 확장으로 원본을 만든다. 연결이 끊기거나 탭을 다시 열어도 서버 오프셋부터 이어서 보내며, 결합 완료 파일을 기존 PocketBase `media.file` 원본으로 등록한 뒤 결합본과 부분 조각을 모두 정리한다. 64MB 미만 영상과 tus 라우트가 없는 이전 서버는 기존 단순 업로드를 쓴다. 임시 조각은 `pb_data` 밖의 `tus-uploads`에 두고 7일 지난 미완료 파일을 정리하며, 최종 등록은 숨김 고유키로 중복 실행을 막는다. `media.file`의 단일 파일 한도 2GB는 그대로 유지한다.
