...
- `posts`: 글. 작성기는 TOAST UI 기반 WYSIWYG Markdown 에디터이고 저장값은 공개 렌더링 호환을 위해 HTML로 저장한다. 공개 보기에서는 table, code block, blockquote, list 같은 Markdown 변환 HTML을 `css/styles.css`의 본문 렌더링 규칙으로 보여준다. 이미지는 본문 안에 직접 넣고, 별도 대표 이미지 로직은 쓰지 않는다.
- `daily_entries`: 나으 하루. 글방과 별도 저장소를 쓰되 같은 WYSIWYG Markdown 작성 경험을 쓰고, 같은 날짜에도 개별 글 레코드로 저장한다. `day_key`는 캘린더와 날짜 정렬용 메타데이터다.
...
- `post_views`: 글별 조회수를 위한 30분 단위 익명 조회 기록. 일반 방문자는 생성만 가능하고, 목록/집계는 로그인한 관리자만 볼 수 있다.
- `media`: 본문 이미지, 영상(MP4/WebM/MOV/M4V), 오디오, PDF 업로드. 글방/나으 하루/프로그램실/About 작성기의 BlockNote 파일 업로드가 이 컬렉션을 쓴다.
...
글 조회수도 같은 30분 세션 철학을 쓴다. 방문자가 `posts/view.html?slug=...`로 글 상세에 들어오면 해당 글의 `post_views` 기록이 1개 생기고, 로그인한 관리자의 조회는 기록하지 않는다. 조회수 숫자는 공개 방문자에게 보이지 않고, owner mode의 글방 목록/글 상세 및 `admin/posts.html`에서만 보인다. 기존 `visitor_sessions`에는 어떤 글을 봤는지 저장하지 않았기 때문에 과거 전체 방문자 수를 글별 조회수로 정확히 되돌려 나눌 수는 없다. 과거 서버 로그를 별도로 수집해 import하지 않는 한, 글별 조회수는 `post_views` 적용 이후부터 쌓이는 값이 기준이다.
...
5. 본문은 같은 WYSIWYG Markdown 에디터에서 작성한다. 본문 이미지는 이미지 버튼, 드래그 앤 드롭, 붙여넣기로 넣고, `/video` 같은 파일 블록은 `media` 업로드를 거쳐 본문에 들어간다. iPhone/Photos에서 나온 MOV/M4V도 허용한다. 유튜브 링크는 붙여넣거나 비디오 블록의 URL 입력으로 넣으면 공개 화면에서 임베드 플레이어로 보인다.
...
4. 본문은 Notion처럼 렌더링되는 WYSIWYG Markdown 화면에서 작성한다. 본문 중간 이미지는 이미지 버튼, 드래그 앤 드롭, 붙여넣기로 넣는다. `/video` 업로드와 유튜브 링크도 같은 본문 안에 넣을 수 있다. 업로드 파일은 `media` 컬렉션에 저장되고 작성기에는 미디어 블록으로 들어간다.
5. 대표 이미지는 따로 정하지 않는다. 본문 이미지는 본문 위치에만 보인다.
6. 상태를 `발행됨`으로 저장
7. 저장할 때 에디터 본문은 HTML로 변환되어 기존 공개 본문 스타일 그대로 홈의 글방 최근 3개 table, 글 목록, 글 상세에 자동 반영된다.
...
4. 글방과 같은 WYSIWYG Markdown 작성 화면에서 제목, 기록 날짜, 본문, 본문 이미지/영상, 유튜브 링크를 입력한다. 기록 날짜는 기본값이 오늘이지만 과거 날짜로 바꿀 수 있다.
5. 저장 시 같은 날짜의 기록이 이미 있어도 새 `daily_entries` 레코드를 만든다.
6. 각 글은 고유 slug를 갖고, `day_key`는 같은 날짜 기록을 캘린더에서 묶어 보여주는 기준으로만 쓴다.
7. 공개 목록은 상단 월간 캘린더와 하단 하루 table로 보인다. 같은 날짜에 기록이 여러 개면 캘린더와 table에는 하루 1개로 묶여 보이고, table row나 캘린더 날짜를 누르면 해당 날짜의 기록들이 시간순으로 펼쳐진 하루 상세로 들어간다.
...
5. 앱 대표 이미지가 있으면 `표지`에 첨부한다. 표지를 비워두고 `파일`에 이미지가 섞여 있거나 `.app.zip` 안에 앱 아이콘이 있으면 자동 표지가 된다. 이미지/앱 아이콘을 찾지 못하면 표지는 `없음` 상태로 보인다.
...
## About / Contact 고치기 흐름

`about.html`은 일반 자기소개 페이지가 아니라 나무위키식 인물 문서처럼 보이는 `김찬수` living profile이다. 방문자는 개요보다 먼저 보이는 프로필 표, 목차, 번호 섹션, 연락처를 읽고, 로그인한 관리자는 같은 공개 페이지 위에서 `OWNER MODE` 편집 도구를 본다.

About 프로필 표와 왼쪽 sidebar의 PROFILE DATA 표는 같은 프로필 row 원본을 기준으로 렌더링된다. About은 문서형 상세 표로, sidebar는 좁은 요약 표로 보이지만 `본명`, `출생`, `국적`, `학력`, `병역`, `링크` 같은 핵심 값은 `site_settings.about_wiki_document`의 `profileRows`를 통해 이어진다. sidebar의 Social Media는 기존처럼 favicon 아이콘 스타일을 유지한다.

1. 홈 상단 marquee의 `coldwaterkim` 텍스트 클릭
2. 로그인 후 `About / Contact`로 이동
3. `OWNER MODE`에서 `섹션 추가`, 섹션별 `[편집]`, `프로필 표 수정`을 사용한다.
4. 섹션 편집은 짧은 `제목` 입력과 글방/나으 하루와 같은 WYSIWYG Markdown `본문` 편집기로 나뉜다. 본문 이미지는 이미지 버튼, 드래그 앤 드롭, 붙여넣기로 넣는다.
5. 섹션 제목을 바꾸면 목차 번호와 링크가 자동으로 다시 만들어진다.
6. 저장된 문서는 별도 컬렉션을 만들지 않고 `site_settings.about_wiki_document` JSON에 저장된다. 그래서 운영 스키마를 바꾸지 않고도 about 문서를 계속 고칠 수 있다.
...
- `daily/view.html`: 날짜별 나으 하루 상세 페이지. `?day=YYYY-MM-DD`로 들어오면 그날 기록만 시간순으로 보여주고, 예전 `?slug=...` 링크도 해당 날짜 상세로 해석한다.
...
- `js/about-wiki.js`: About / Contact의 위키형 문서 렌더링과 owner 섹션/프로필 표 편집
- `js/profile-data.js`: sidebar PROFILE DATA와 About 프로필 표가 공유하는 기본 프로필 row, legacy row 보정, sidebar 렌더링
- `programs/index.html`: CMS-backed 프로그램실 페이지
- `programs/view.html`: 개별 프로그램 상세페이지
- `nasajab/index.html`: CMS-backed 나사잡 페이지
- `admin/daily.html`: 글방 작성기와 같은 WYSIWYG Markdown 경험으로 `daily_entries`를 쓰고, 같은 날짜라도 새 글은 개별 레코드로 저장하는 owner 화면
...
- `js/markdown-editor.js`: 글방, 나으 하루, 프로그램실, About owner 본문 작성기가 공유하는 BlockNote WYSIWYG Markdown 래퍼. 이미지 버튼, `/video` 파일 업로드, 유튜브 링크 붙여넣기, 공개 HTML 보정의 출발점이다.
- `js/media-embeds.js`: 저장된 본문 HTML의 video/audio 컨트롤과 유튜브 임베드 변환을 맡는 가벼운 공개 렌더링 보정 모듈
