import{C as e,I as t,U as n,V as r,X as i,g as a,j as o,ot as s,r as c,v as l,y as u}from"../pb-D2QCuDy_.js";import"../site-DnU6m-Az.js";var d=new URLSearchParams(window.location.search).get(`slug`),f=r();d?p(d):x(`글을 찾을 수 없습니다.`,`Post not found`);async function p(r){try{b(r);let i=await o(r,f);if(i.status!==`published`&&!f){x(`이 글은 아직 발행되지 않았습니다.`,`Not published`);return}document.title=`${i.title} — coldwaterkim`,document.getElementById(`timeline-title`).textContent=`글방`,document.getElementById(`timeline-note`).innerHTML=`<b>${l(i.title)}</b>`;let s=m(f?await e():await t(),i);f&&(document.getElementById(`owner-tools`).innerHTML=`
                        <div class="owner-bar">
                            <b>OWNER MODE</b> ·
                    ...
                    `,document.getElementById(`deletePostLink`).addEventListener(`click`,async e=>{if(e.preventDefault(),confirm(`이 글을 삭제할까요? 이 작업은 되돌릴 수 없습니다.`))try{await a(i.id),window.location.href=`index.html`}catch(e){alert(`삭제 실패: `+c(e))}}),document.getElementById(`logoutLink`).addEventListener(`click`,e=>{e.preventDefault(),n(),window.location.reload()})),h(s,i.id),_(i.id),v(i.id)}catch(e){x(c(e),`Post not found`)}}function m(e,t){return s(e.some(e=>e.id===t.id)?e:[...e,t])}function h(e,t){let n=document.getElementById(`post-timeline`);if(e.length===0){n.innerHTML=`<p>아직 글이 없습니다.</p>`;return}n.innerHTML=e.map(e=>g(e,e.id===t)).join(``)}function g(e,t){let n=i(e),r=e.status===`published`?``:` <small class="note">[초안]</small>`,a=t?`<div class="timeline-focus-badge">★ NOW READING ★</div>`:``,o=f?`
