...
공개 메인 IA 페이지(`Home`, `글방`, `글 상세`, `프로그램실`, `나사잡`, `Guestbook`, `About / Contact`)는 모두 홈의 2-column shell을 기본 레이아웃으로 쓴다. 즉 상단 marquee, 노란 visitor banner, 왼쪽 프로필/sidebar, 오른쪽 content 상단 navigation은 유지하고, 페이지별 내용만 오른쪽 content 영역에서 바뀌게 한다.
...
모바일에서는 같은 shell을 유지하되, 640px 이하 화면에서만 메인 2-column table이 세로로 접힌다. Home은 프로필을 가로형 미니 명함으로 압축하고 Cool Links/My WebRing은 본문 하단으로 보내 정체성을 남긴다. 글방/글 상세/프로그램실/나사잡/Guestbook/About는 sidebar를 숨겨 navigation 다음에 본문이 바로 오게 한다. 데스크톱의 760px PC 레이아웃은 기본값으로 보존한다.
...
- 글 상세 URL은 홈과 같은 shell 안에서 `slug`로 PocketBase 글을 조회하되, 단일 글만 보여주지 않고 전체 글을 최신순 timeline으로 렌더링한다. 특정 글 링크로 들어오면 해당 글이 `NOW READING` 상태로 첫 포커스를 받고, 그 위에는 더 최근 글, 아래에는 더 오래된 글이 이어진다.
- 프로그램실은 홈과 같은 shell 안에서 PocketBase `programs` 컬렉션을 읽어 table 자료실 형태로 보여준다. 첫 화면의 table row는 preview이며, 표지/이름/`상세 이야기 보기`를 누르면 `programs/view.html?slug=...` 상세페이지로 들어간다. 각 프로그램은 표지, 상태, 플랫폼, 만든 이유, 해결하는 페인포인트, 받기 링크를 같은 row 안에 담고, 1시간짜리 실험작과 장기 프로젝트를 같은 무게로 취급한다. 표지는 `cover_image`가 있으면 업로드 이미지를 쓰고, 없으면 프로그램명/상태 기반 레트로 표지를 자동 생성한다. `.dmg`, `.zip` 같은 파일은 `download_files`에 직접 첨부하고, TestFlight/GitHub/Web Demo는 링크 필드로 넣는다.
- 프로그램 상세페이지는 같은 shell 안에서 긴 스토리(`story_detail`), 해결 방식(`solution`), 제작 노트(`build_notes`), 스크린샷 갤러리(`screenshots`), 다운로드/링크를 보여준다. 목록 table이 자료실 인덱스라면 상세페이지는 개별 작품 페이지다.
- 나사잡은 홈과 같은 shell 안에서 PocketBase `nasajab` 컬렉션을 읽어 “나를 사로 잡은 것들”을 보여준다. 상단에는 최신 항목 또는 아카이브에서 선택한 항목 1개를 `TODAY'S 나사잡` table로 크게 보여주고, 아래에는 최신 항목을 제외한 항목을 5개씩 `No. / Thumbnail / 메모 / 날짜` 아카이브 table로 쌓는다. 제목과 내용은 나누지 않고 `memo` 하나만 저장한다. 아카이브 메모/썸네일을 누르면 같은 페이지 상단 큰 영역에서 확대되어 보인다.
...
- 로그인한 관리자는 공개 사이트를 그대로 보면서 홈 문구 편집, 프로필 사진 클릭 업로드, 홈 BGM MP3 업로드, 글방의 새 글/수정/삭제, 글 상세의 수정/삭제, 프로그램실의 새 프로그램/수정/삭제, 나사잡의 이미지 업로드/수정/삭제, 방명록 삭제 같은 `OWNER MODE` 권한을 추가로 본다. 프로그램실과 나사잡에서는 `공개` 체크가 켜진 레코드만 방문자에게 보인다.
- 프로필 사진과 홈 BGM은 별도 컬렉션을 만들지 않고 기존 `media` 컬렉션에 파일을 올린 뒤 `site_settings.profile_photo_url`, `site_settings.bgm_audio_url`, `site_settings.bgm_audio_title`에 현재 선택값을 저장한다. BGM 제목은 미니 플레이어 위에서 marquee처럼 흐르고, 오디오는 접속 시 자동재생을 시도하되 브라우저가 막으면 첫 사용자 입력 뒤 다시 시도한다.
...
- 프로그램실 공개 API 목록 조회 5개 확인, 임시 프로그램 create/update/delete와 `cover_image`/`download_files`/`screenshots`/상세 스토리 필드 저장 검증 완료
