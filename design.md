...
관리 경험은 공개 사이트와 분리된 대시보드가 아니라 `OWNER MODE` 레이어로 본다. 방문자는 같은 페이지를 읽고, 로그인한 주인장만 같은 화면 위에서 편집/작성/삭제 버튼을 추가로 본다. Home에는 글방/나으 하루/프로그램실 새 작성을 시작하는 `통합 글쓰기` 진입점을 하나 두되, 기존 각 페이지별 작성/수정 흐름도 유지한다.
...
글방과 프로그램실의 새 콘텐츠 URL slug는 제목의 한글·영문·숫자를 유지하고 공백·구두점을 하이픈으로 정리한다. 이미 발행된 콘텐츠의 slug는 바꾸지 않아 기존 링크를 보존한다.
...
- 표지는 `cover_image` 업로드가 있으면 그 이미지를 우선한다. 없으면 `download_files`에 첨부된 첫 이미지 파일이나 업로드 시 `.app.zip`에서 추출한 앱 아이콘을 자동 표지로 쓰고, 이미지/앱 아이콘이 없으면 표지는 `없음` 상태로 둔다.
...
`Ask Me`는 방명록의 인사 공간과 분리된 질문함이다. 입력부는 시각적 label 없이 질문 `textarea`, `이 질문은 공개하지 않기` checkbox, 기본 submit button만 둔다. 닉네임·제목·카테고리·이메일·첨부파일·댓글·좋아요는 받지 않는다. 홈에는 최근 글 8개와 앨범 사이에 같은 최소 질문창을 두고, 바로 아래에 작성 순서 기준 최신 질문 3개를 답변 여부와 관계없이 붙인다. 별도 카드로 분리하지 않고 주변 섹션처럼 회색 헤더, table row, 각진 폼 문법을 유지한다. 각 행은 Ask Me의 해당 질문으로 이동하며, 공개 답변은 `Q.`와 남색 double border-left의 `A.`를 짧게 보여준다. 미답변·비공개 질문은 공개 feed의 상태 문구만 표시하고 홈에서는 비밀번호 입력이나 실제 원문 열람을 제공하지 않는다. 헤더에는 `Ask Me`와 `질문 목록` 링크를 두고 placeholder는 `질문이 있다면`이다. Ask Me 본문 제목은 `???`를 쓰되 상단 탐색 메뉴의 `Ask Me` 명칭은 유지한다.
